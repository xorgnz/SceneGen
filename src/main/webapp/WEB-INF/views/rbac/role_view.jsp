...
<fieldset>
    ...
    <label>ID</label>               <input type="text" value="${role.id}"           disabled="disabled" />
    <label>Name</label>             <input type="text" value="${role.name}"         disabled="disabled" /><br/>
    <label>Description</label>      <textarea disabled="disabled">${role.description}</textarea>
</fieldset>
</div>
...
<!-- Form - Assign new Permission -->
<c:choose>
<c:when test="${fn:length(allPermissions) > 0}">
<form:form method="post" action="rbac/role_perm_link/assign">
    <fieldset>
        ...
        <button type="submit">Assign this Permission</button>
    </fieldset>
</form:form>
</c:when>
</c:choose>
...
<tr>
    <td><a href="rbac/permission/${permission.id}" target="frame-main">${permission.name}</a></td>
    <td>${permission.description}</td>
    <td>
        <a href="rbac/permission/${permission.id}/edit" target="frame-main">Edit</a>&#160;
        <a href="rbac/permission/${permission.id}/delete" target="frame-main">Delete</a>&#160;
