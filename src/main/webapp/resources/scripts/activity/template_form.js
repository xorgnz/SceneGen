...
var createParameterLine = function (parameter)
{
    ...
    // Component - Label
    var input_label = DOMUtils.input("pl", parameter.label ? parameter.label : "", "text", "width:220px");
    var td_label = document.createElement("td");
    td_label.appendChild(input_label);
    
    // Component - Variable
    var input_variable = DOMUtils.input("pv", parameter.variable ? parameter.variable : "", "text", "width:220px");
    var td_variable = document.createElement("td");
    td_variable.appendChild(input_variable);
    
    // Component - Type
    var input_type = DOMUtils.input("pv", parameter.type ? parameter.type : "", "text", "width:220px");
    var td_type = document.createElement("td");
    td_type.appendChild(input_type);
    ...
    for (var i = 0 ; i < rows.length ; i++ )
    {
        ...
        if (fields && fields.variable.value)
        {
            if (! variables[fields.variable.value])
            {
                ...
                // Field - Type
                var hidden_type = DOMUtils.input("param_type", fields.type.value, "hidden");
                tbody.appendChild(hidden_type);
