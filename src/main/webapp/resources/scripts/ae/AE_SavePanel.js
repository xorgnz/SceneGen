...
**************************************************************************/
var AE_SavePanel = function (container, config, persistor)
{
    // Save components
    ...
    UI_Panel.embellish(this, "AE_SavePanel");
...
AE_SavePanel.prototype.buildUI = function ()
{
    // Clear contents
    this.clear();
    ...
    this.container.appendChild(this.buildUI_loadScene(scenes));
...
AE_SavePanel.prototype.buildUI_loadScene = function (scenes)
{
    // Self reference
    var that = this;

    // Container div
    var div = DOMUtils.div("", "loadScene");

    // Component - Scene selector
    scenes = _.sortBy(scenes, function (o) { return o.name; });
    this.htmlElements.select_scene_load = DOMUtils.select(
        "scene_load",
        scenes,
	...
	var button = DOMUtils.button("","","Load scene");
    button.onclick = function ()
    {
        $(button).hide();
        $(div_working_notification).show().text("Working...");

        that.persistor.loadScene(
            that.htmlElements.select_scene_load.value,
            function()
            {
                $(button).show();
                $(div_working_notification).text("Scene '" + that.persistor.sceneName + "' loaded").delay(5000).fadeOut();
            });
    };
    ...
    // Assemble Section
    div.appendChild(DOMUtils.header(2, "Load scene contents into explorer"));
    var div_glue = DOMUtils.div("glue");
    div_glue.appendChild(this.htmlElements.select_scene_load);
...
AE_SavePanel.prototype.buildUI_saveExistingScene = function (scenes)
{
    // Self reference
    var that = this;
...
AE_SavePanel.prototype.buildUI_saveNewScene = function ()
{
    // Self reference
    var that = this;
