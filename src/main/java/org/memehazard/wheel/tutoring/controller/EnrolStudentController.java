...
@Controller
@RequestMapping("/tutoring/studentModel/enrol")
public class EnrolStudentController
{
            ...
            HttpServletRequest request)
    {
        ...
        if (u == null)
            flash.addFlashAttribute("msg_bad", "Unable to enrol using user with ID " + studentId + " - does not exist");
        else if (c == null)
            flash.addFlashAttribute("msg_bad", "Unable to enrol using curriculum with ID " + studentId + " - does not exist");
        else
        {
            facade.enrolStudentInCurriculum(studentId, curriculumId);
            flash.addFlashAttribute("refresh", "frame-side");
            flash.addFlashAttribute("msg_good", String.format("Enroled user \"%s\" in curriculum \"%s\"", u.toNameString(), c.getName()));
        }
