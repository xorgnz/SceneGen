...
@Controller
@RequestMapping("/rbac/role_perm_link/deassign")
public class DeassignRolePermLinkController
{
    @Autowired
    ...
    {
        // Act
        facade.deassignPermissionFromRole(permId, roleId);

        // Response message
        Role r = facade.getRole(roleId);
        Permission p = facade.getPermission(permId);
        flash.addFlashAttribute("msg_good", String.format("Removed permission \"%s\" from role \"%s\"", p.getName(), r.getName()));
