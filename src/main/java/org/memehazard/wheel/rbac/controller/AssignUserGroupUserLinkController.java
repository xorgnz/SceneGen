...
@Controller
@RequestMapping("/rbac/usergrp_user_link/assign")
public class AssignUserGroupUserLinkController
{
    @Autowired
    ...
    {
        // Act
        facade.assignUserToUserGroup(userId, userGroupId);

        // Response message
        User u = facade.getUser(userId);
        UserGroup ug = facade.getUserGroup(userGroupId);
        flash.addFlashAttribute("msg_good", String.format("Assigned user \"%s\" to group \"%s\"", u.toNameString(), ug.getName()));
