...
@Controller
@RequestMapping("/rbac/user_role_link/deassign")
public class DeassignUserRoleLinkController
{
    @Autowired
    ...
    {
        // Act
        facade.deassignRoleFromUser(roleId, userId);

        // Response message
        User u = facade.getUser(userId);
        Role r = facade.getRole(roleId);
        flash.addFlashAttribute("msg_good", String.format("Removed role \"%s\" from user \"%s\"", r.getName(), u.toNameString()));
